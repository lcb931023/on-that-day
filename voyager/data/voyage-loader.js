...
for (const m of MILESTONES) {
  ...
  for (let k = 1; k <= need; k++) {
    ...
    const pos = m.lat != null ? { lat: m.lat, lng: m.lng } : coordAt(date);
    synthetic.push({
...
for (const [date, line] of Object.entries(HIGHLIGHTS)) {
  ...
  const c = classify(date);
  const pos = c.lat != null ? { lat: c.lat, lng: c.lng } : coordAt(date);
  existingDates.add(date);
...
export const TIMELINE = [...journalDays, ...synthetic].sort((a, b) => (a.date < b.date ? -1 : 1));
